...
export const verifyPayment = async (paymentId) => {
  ...
  // created_at is Tabby's own, immutable timestamp for the payment —
  // confirmed against a real sandbox payment (e.g. "2026-09-18T00:48:37Z").
  // It is the only stable timestamp Tabby returns: there is no
  // authorized_at, and `captures` is empty until capture. Returning it lets
  // paid_at be anchored to the payment rather than to whenever it happened
  // to be verified.
  return { id: payment.id, status: payment.status, amount: payment.amount, currency: payment.currency, createdAt: payment.created_at ?? null };
};

// TEMPORARY diagnostic (same pattern as the removed CJ/Tabby discovery
// helpers) — never throws, returns the raw status/body so a real failure
// reason can be inspected instead of guessed. Remove once verifyPayment is
// confirmed against a real sandbox payment.
export const tabbyRawGet = async (path) => {
  guardTestMode();
  if (!process.env.TABBY_SECRET_KEY) throw new Error('TABBY_SECRET_KEY is not configured');
  const response = await fetch(`${TABBY_BASE_URL}${path}`, {
    method: 'GET',
