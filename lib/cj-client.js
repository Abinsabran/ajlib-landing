...
//
// Therefore, when this is eventually enabled, the caller MUST NOT wrap it in
// a blind retry. A timeout is not a failure — the money may already be gone.
// Every timeout, and every repeated-payment business error, must be resolved
// through lib/cj-fulfillment.js reconcileAfterTimeout(), which is read-only
// and only reports safeToRetry=true when CJ has no order under our
// deterministic orderNumber at all.
//
...
// guard, balance preflight, idempotency check) without ever invoking it.
