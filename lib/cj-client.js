...
export const calculateFreight = async ({ startCountryCode, endCountryCode, vid, quantity, products }) => {
  const accessToken = await getAccessToken();
...
export const queryProductConnections = async ({ shopId, platformProductId, page = 1, pageSize = 100 } = {}) => {
  const accessToken = await getAccessToken();
