...
//
// LIVE STATUS (confirmed, read-only): this exact documented URL currently
// returns {"code":1600101,"message":"Interface not found","result":false}
// for AJLIB's CJ account. The path is byte-identical to CJ's published one,
// so the endpoint appears not to be enabled for this account rather than
// mistyped. No alternative path is guessed here. Until CJ enables it,
// parseCjBalanceUSD yields null and the balance preflight BLOCKS automatic
// fulfillment (CJ_BALANCE_UNAVAILABLE) instead of assuming funds exist.
export const getAccountBalance = async () => throttleCj(async () => {
  const accessToken = await getAccessToken();
  const response = await fetch(`${CJ_BASE_URL}/shopping/balance/getBalance`, {
    method: 'GET',
    headers: { 'CJ-Access-Token': accessToken, 'Content-Type': 'application/json' }
  });
  const body = await response.json().catch((e) => ({ parseError: e.message }));
