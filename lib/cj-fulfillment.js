...
import { calculateFreight, queryProductConnections, getAccountBalance, isCjErrorBody, CJ_RATE_LIMITED_CODE } from './cj-client.js';
import {
  ...
  STRIPE_FEE_PERCENT, STRIPE_FEE_FIXED_AED, STRIPE_INTERNATIONAL_SURCHARGE_PERCENT, TABBY_FEE_PERCENT,
...
export const paymentFeeUSD = ({ amountCollectedFils, provider = 'stripe', international = false }) => {
  ...
  if (provider === 'tabby') {
    ...
    return aedToUsd(amountAed * (TABBY_FEE_PERCENT / 100));
  }
...
export const minimumRevenueAedForMargin = ({
  ...
  const fixedAed = isTabby ? 0 : STRIPE_FEE_FIXED_AED;
...
export const prepareFulfillment = async (orderRow, {
  ...
  const margin = evaluateFulfillmentMargin({
    ...
    aedToUsdRate,
    minAcceptableMarginPercent,
    unitCount: resolved.reduce((sum, item) => sum + item.quantity, 0),
    ...
    customizationCostPerUnitUSD: additionalFulfillmentCostUSD
  });
  if (!margin.approved) {
