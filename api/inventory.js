...
export default async function handler(req,res){
  try{
    if(req.method==='GET'){res.setHeader('Cache-Control','private, no-store, max-age=0');const r=await db('/rest/v1/inventory?select=color,size,stock,track_stock,allow_preorder,preorder_eta,updated_at&order=color,size');return res.status(r.status).json(await r.json())}
