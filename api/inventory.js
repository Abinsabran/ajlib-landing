...
export default async function handler(req,res){
  try{
    ...
    if(req.method==='GET'){const r=await db('/rest/v1/inventory?select=color,size,stock,track_stock,updated_at&order=color,size');return res.status(r.status).json(await r.json())}
    if(req.method!=='PATCH')return res.status(405).json({error:'Method not allowed'});
    const {color,size,stock,track_stock}=req.body||{};
    ...
    const r=await db(`/rest/v1/inventory?color=eq.${encodeURIComponent(color)}&size=eq.${encodeURIComponent(size)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({stock:Number(stock),track_stock:Boolean(track_stock),updated_at:new Date().toISOString()})});
